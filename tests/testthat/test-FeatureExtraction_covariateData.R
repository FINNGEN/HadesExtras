...
test_that("covariateData_YearOfBirth works", {
  ...
  withr::defer({DatabaseConnector::dropEmulatedTempTables(connection); DatabaseConnector::disconnect(connection)})
  ...
  covariate_control$covariates |> dplyr::collect() |> dplyr::distinct(covariateId) |>
    dplyr::pull(covariateId) |> expect_equal(1041)
})
...
test_that("covariateData_YearOfBirth works aggregated", {
  ...
  covariate_control$covariatesContinuous  |> dplyr::collect() |> dplyr::pull(covariateId)  |> expect_equal(1041)
