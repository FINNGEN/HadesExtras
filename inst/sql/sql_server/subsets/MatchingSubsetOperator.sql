...
FROM(
  ...
  FROM (
    ...
    {@require_match_observed_at_target_start_date == '1'} ? {AND ( intput_to_match.cohort_start_date <= target_matching_rules.cohort_start_date AND target_matching_rules.cohort_start_date <= intput_to_match.cohort_end_date)}
  ) AS ST
