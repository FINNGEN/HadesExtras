...
CohortGenerator_MatchingSubsetOperator <- R6::R6Class(
  ...
  public = list(
    ...
    getAutoGeneratedName = function() {
      ...
      if (private$.matchSex) {
        matchBy <- append(matchBy, "sex")
      }
      if (private$.matchBirthYear) {
        matchBy <- append(matchBy, "birth year")
      }
...
CohortGenerator_OperationSubsetQb <- R6::R6Class(
  classname = "OperationSubsetQb",
  inherit = CohortGenerator:::QueryBuilder,
  private = list(
    innerQuery = function(targetTable) {
      sql <- operationStringToSQL(private$operator$operationString)
...
# MatchingSubsetOperator ------------------------------
#' @title Operation Subset Operator
#' @export
#' @description
#' A subset of type cohort - subset a population to only those contained within defined cohort
CohortGenerator_OperationSubsetOperator <- R6::R6Class(
  classname = "OperationSubsetOperator",
  inherit = CohortGenerator::SubsetOperator,
  private = list(
    suffixStr = "Operation",
    queryBuilder = CohortGenerator_OperationSubsetQb,
    .operationString = character(0)
  ),
  public = list(
    #' to List
    #' @description List representation of object
    toList = function() {
      objRepr <- super$toList()
      objRepr$operationString <- private$.operationString
    ...
    #' @return character
    getAutoGeneratedName = function() {
      nameString <- paste0("Operation: ", private$.operationString)

      return(paste0(nameString))
    }
  ),
  active = list(
    #' @field operationString Integer id of cohorts to match to
    operationString = function(operationString) {
      if (missing(operationString)) {
        return(private$.operationString)
      }

      operationString <- as.character(operationString)
      # TODO: assert string is valid
      checkmate::assertString(operationString)
...
#'
#' @param name  optional name of operator
#' @param operationString string - the operation string to be used in the query
#'
#' @returns a OperationSubsetOperator instance
createOperationSubset <- function(
    name = NULL,
    operationString) {
  subset <- CohortGenerator_OperationSubsetOperator$new()
  subset$name <- name
  subset$operationString <- operationString
